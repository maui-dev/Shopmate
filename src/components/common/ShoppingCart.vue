<template>
  <section class="shopping__cart" :class="{'is__shopping': displayStatus}">
      <button class="shopping__cart-closebtn">
        <a><img @click="closeCart" src="@/assets/img/closebtn.svg" alt="Close Button" ></a>
      </button>
      <div class="shopping__cart-productscontainer" v-if="cartItems.length > 0">
        <h2 class="shopping__cart-count">{{cartItems.length}} {{cartItems.length > 1 ? 'Items' : 'Item'}} in Your Cart</h2>
        ...
        <ul class="shopping__cart-products">
          <li :key="item.item_id" v-for="item in cartItems" class="shopping__cart-product shopping__cart--product1">
            <div class="shopping__cart-details">
              <img :src="$store.state.imagesEndpoint+'/'+item.image" alt="Shirt image" class="shopping__cart-details--image">
              <div class="shopping__cart-details--infos">
                <h3 class="shopping__cart-details--infos-name">{{item.name}}</h3>
                <a @click.prevent="removeCartItem(item.item_id)" class="shopping__cart-details--infos-removebtn">
            ...
            <div class="shopping__cart-quantity">
              <div class="product__buttons">
                <button class="rounded product__decrement" @click.prevent="handleCountingAction(item.quantity, 'minus', item.item_id)">
                  <img src="@/assets/img/minus.svg" alt="minus button">
                </button>
                <span class="product__quantity-number">{{item.quantity}}</span>
                <button class="rounded product__increment" @click.prevent="handleCountingAction(item.quantity, 'plus', item.item_id)">
